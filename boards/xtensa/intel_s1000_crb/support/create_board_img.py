...
create_spi_flash.py -i in_file.bin -o out_file.bin
                    [-m|--no_sram] [-c|--no_l1cache] [-t|--no_tlb]
                    [-x|--no_exec] [-s|--no_sha] [-k|--clk_sel]
"""
...
def ipc_load_fw(load_addr, fw_size, fw_offset):
    dword_count = 3;
    load_flags = 0;
    clock_sel = 0;
    ...
    flash_content.append(load_addr)
...
def main():
    ...
    # workaround for sram retention
    ipc_dbg_memwrite(0x304628, 0xd)

    # workaround for memory hole error
    ipc_dbg_memwrite(0x71fd0, 0x3)

    ipc_load_fw(FW_LOAD_ADDR, SRAM_SIZE, FLASH_PART_TABLE_OFFSET)
